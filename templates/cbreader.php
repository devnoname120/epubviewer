...
<head class="session" data-nonce='<?php p($nonce); ?>' data-downloadlink='<?php print_unescaped($downloadLink); ?>'
  ...
  <link rel="stylesheet"
        href="<?php p($urlGenerator->linkTo('epubviewer', 'js/cbrjs/css/cbr.css')) ?>?v=<?php p($version) ?>">
  ...
  <script type="text/javascript" nonce="<?php p($nonce) ?>"
    ...
    <?php if ($idevice): ?>
      <link rel="stylesheet"
            href="<?php p($urlGenerator->linkTo('epubviewer', 'js/cbrjs/css/idevice.css')) ?>?v=<?php p($version) ?>">
    <?php endif; ?>
...
<div id="progressbar" style="display:none;">
  ...
  <br>
  <div class="message"><span class="message-icons"><span class="icon-cloud_download"></span><span
        class="icon-unarchive"></span></span> <span class="message-text"></span></div>
</div>
<!-- /loading progressbar -->
...
<div class="toolbar control" name="toolbar">
  ...
  <div class="pull-right">
    <div>
      <button data-trigger="click" data-action="toggleLayout" title="toggle one/two pages at a time"
              class="icon-single_page_mode layout layout-single"></button>
      <button data-trigger="click" data-action="toggleLayout" title="toggle one/two pages at a time"
              class="icon-double_page_mode layout layout-double"></button>
    </div>
    <div>
    ...
    <div>
      <button data-trigger="click" data-action="fitWidth" title="fit page to window width"
              class="icon-icon-fit-width"></button>
    </div>
    <div>
      <button data-trigger="click" data-action="fitWindow" title="fit page to window"
              class="icon-icon-fit-window"></button>
    </div>
    <div>
      <button data-trigger="click" data-action="toggleReadingMode" title="switch reading direction"
              class="icon-format_textdirection_l_to_r manga-false"></button>
      <button data-trigger="click" data-action="toggleReadingMode" title="switch reading direction"
              class="icon-format_textdirection_r_to_l manga-true"></button>
    </div>
    <div>
      <button data-trigger="click" data-action="toggleFullscreen" title="toggle fullscreen"
              class="icon-fullscreen fullscreen-false"></button>
      <button data-trigger="click" data-action="toggleFullscreen" title="toggle fullscreen"
              class="icon-fullscreen_exit fullscreen-true"></button>
    </div>
...
<!-- busy overlay -->
<div id="cbr-busy-overlay" class="cbr-control control overlay" name="busyOverlay" data-trigger="click"
     data-action="closeSidebar" style="display:none"></div>
<!-- /busy overlay -->

<!-- navigation -->
<div data-trigger="click" data-action="navigation" data-navigate-side="left"
     class="cbr-control navigate navigate-left control" name="navigateLeft">
...
-->
<div data-trigger="click" data-action="navigation" data-navigate-side="right"
     class="cbr-control navigate navigate-right control" name="navigateRight">
...
<div class="sidebar control" name="sidebar" id="sidebar">
  <div class="panels">
    <div class="pull-left">
      <button data-trigger="click" data-action="showToc" title="Table of Contents"
              class="icon-format_list_numbered toc-view open"></button>
      <button data-trigger="click" data-action="showBookSettings" title="Book settings"
              class="icon-rate_review book-settings-view"></button>
      <button data-trigger="click" data-action="showSettings" title="Default settings"
              class="icon-settings settings-view"></button>
    </div>
    <div class="pull-right">
      <button id="toc-populate" data-trigger="click" data-action="tocPopulate" title="generate thumbnails"
              class="icon-sync" style="display:none"></button>
  ...
  <div class="book-settings-view view">
    <div class="metadata">
      <table>
        ...
        <tr>
          <td>Page count:</td>
          <td class="book-pagecount"></td>
        </tr>
        <tr>
          <td>Size:</td>
          <td class="book-size"></td>
        </tr>
      </table>
    </div>
      ...
      <form name="image-enhancements" data-trigger="reset" data-action="resetEnhancements">
        <div class="sliders">
          <div class="control-group">
            ...
            <input id="brightness" data-trigger="change" data-action="brightness" type="range" min="-100" max="100"
                   step="1" value="0">
          </div>
          <div class="control-group">
            <label title="adjust contrast" class="icon-contrast"></label>
            <input id="contrast" data-trigger="change" data-action="brightness" type="range" min="-1" max="1" step="0.1"
                   value="0">
          </div>
          <div class="control-group">
            <label title="sharpen" class="icon-droplet"></label>
            <input id="sharpen" data-trigger="change" data-action="sharpen" type="range" min="0" max="1" step="0.1"
                   value="0">
          </div>
        </div>
  ...
  <div class="settings-view view">
    <div class="settings-container" name="thumbnail-settings" id="thumbnail-settings">
      ...
      <div class="control-group pull-left">
        ...
        <input id="thumbnail-width" data-trigger="change" data-action="thumbnailWidth" type="number" min="50" max="500"
               step="10" value="200">
    ...
    <div class="settings-container" name="sidebar-settings" id="sidebar-settings">
      ...
      <form name="sidebar-preferences" data-trigger="reset" data-action="resetSidebar">
        ...
        <div class="control-group pull-left">
          ...
          <input id="sidebar-width" data-trigger="change" data-action="sidebarWidth" type="number" min="5" max="100"
                 step="1" value="20">
