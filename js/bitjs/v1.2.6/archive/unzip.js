...
/**
 * Reads a ZIP string while honoring the language encoding flag from APPNOTE.TXT.
 * Unset names retain BitJS's byte-for-byte legacy decoding for compatibility.
 * @param {ByteStream} bstream
 * @param {number} length
 * @param {boolean} utf8
 * @returns {string}
 */
function readZipString(bstream, length, utf8) {
  if (!utf8) {
    return bstream.readString(length);
  }

  return new TextDecoder('utf-8').decode(bstream.readBytes(length));
}
...
class ZipLocalFile {
  ...
  constructor(bstream) {
    ...
    if (this.fileNameLength > 0) {
      this.filename = readZipString(
        bstream,
        this.fileNameLength,
        (this.generalPurpose & BIT[11]) !== 0,
      );
    }
...
async function archiveUnzip() {
  ...
  if (bstream.peekNumber(4) == CENTRAL_FILE_HEADER_SIG) {
    ...
    while (bstream.peekNumber(4) == CENTRAL_FILE_HEADER_SIG) {
      ...
      const utf8 = (cdfh.generalPurposeBitFlag & BIT[11]) !== 0;
      cdfh.fileName = readZipString(bstream, cdfh.fileNameLength, utf8);
      cdfh.extraField = bstream.readString(cdfh.extraFieldLength);
      cdfh.fileComment = readZipString(bstream, cdfh.fileCommentLength, utf8);
      if (logToConsole) {
